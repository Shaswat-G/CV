...
\resumeSubHeadingListStart
      ...
      {Master's Thesis (Research): Adaptive multi-objective query routing in heterogeneous engines}{}
            \resumeItemListStart
                \resumeItem{Engineered cross-engine performance profiling system for distributed query engines (Spark, Presto, PostgreSQL) with custom instrumentation layers capturing compute, memory, and I/O metrics at query-level granularity}
                \resumeItem{Designed reproducible benchmarking framework for cost model training; executed 220 experiments (22 queries × 5 replicates × 2 scales) with controlled parallelism configurations to generate training data for learned optimizer}
            \resumeItemListEnd
