...
\resumeSubHeadingListStart
    ...
    \resumeSubheading
      {IBM Research Europe — Zurich, flex.data Group}{Sep 2024 -- Present}
      {Master's Thesis in collaboration with \textbf{Systems Group @ ETH} (Prof. Gustavo Alonso)}{}
    \resumeItemListStart
        \resumeItem{Developing cross-platform query optimizer using Graph-Neural-Networks to learn universal, engine-agnostic query representations}
        \resumeItem{Designed multi-task learning framework with GNN encoder \& engine-specific predictors for few-shot adaptation to new engines}
        \resumeItem{Built production-grade engine-native profiling infrastructure for Apache Spark, Presto, and PostgreSQL}
    \resumeItemListEnd
...
\begin{itemize}[leftmargin=0.15in, itemsep=-2pt]  
  ...
  \item \textbf{Machine Learning:} PyTorch, TensorFlow, scikit-learn, XGBoost, Computer Vision, NLP
  ...
  \item \textbf{Performance Engineering:} Performance Profiling, Distributed Training (Spark, Ray), HPC Optimization, Model Parallelization
\end{itemize}
}
